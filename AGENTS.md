...
test/
  unit/                  Unit tests (node:test, dependency-injected, no VS Code API)
    ...
    initializeProject.test.ts  Status display, agents file classification, formatError (43 tests)
    ...
    patchloomCli.test.ts Patchloom CLI integration with real binary + managed install e2e MCP (44 tests incl. e2e)
    propertyBased.test.ts  Property-based tests with fast-check (13 tests)
    quickActions.test.ts Quick action command building, path containment, patch merge (62 tests)
